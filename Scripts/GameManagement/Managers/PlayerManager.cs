...
public class PlayerManager : MonoBehaviour
{
    ...
    private void InitializePlayerData(GameObject player, GameObject spawnRoom)
    {
        ...
        {
            // Set current room reference using position tracker
            positionTracker.SetcurrentRoom(spawnRoom.GetComponent<RegularRoom>());
        }
        else
        {
            Debug.LogError($"UnitPositionTracker component missing on player {player.name}");
        }

        if (identity != null)
        {
            // Set name and level using identity component
            identity.SetName(player.name);
            identity.SetLevel(1);
        }
        else
        {
            Debug.LogError($"UnitIdentity component missing on player {player.name}");
        }

        if (memoriaSystem != null)
        {
            // Initialize memoria using memoria system
            memoriaSystem.SetCurrentMemoria(0);
            memoriaSystem.SetTotalMemoria(0);
        }
        else
        {
            Debug.LogError($"MemoriaSystem component missing on player {player.name}");
        }
    }
    ...
    private void SetPlayerRotation(GameObject player)
    {
        ...
        // Retrieve the current room from the player's UnitPositionTracker component
        UnitPositionTracker positionTracker = player.GetComponent<UnitPositionTracker>();
        if (positionTracker == null || positionTracker.GetCurrentRoom() == null)
        {
            Debug.LogWarning("Player has no current room set");
            return;
        }

        // Get the name of the room GameObject
        string roomName = positionTracker.GetCurrentRoom().gameObject.name;
