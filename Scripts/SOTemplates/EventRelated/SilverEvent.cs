...
[CreateAssetMenu(fileName = "New Silver Event", menuName = "Twilight's Messiah/Events/Encounter/Silver Event")]
public class SilverEventSO : EventTypeSO
{
    ...
    {
        Debug.Log($"Silver Event triggered by {player.name}!");
        ...
        if (selectedEnemy == null)
        {
            ...
            if (selectedEnemy == null)
            {
                Debug.Log("No enemies left in this Silver Event's pool, not even challengers!");
                return;
            }
    ...
    private void HandleBattleComplete(GameObject player, GameObject enemy, bool playerWon)
    {
        ...
        if (playerWon)
        {
            Debug.Log("Player won the Silver Event battle!");
        ...
        {
            Debug.Log("Player lost the Silver Event battle!");
