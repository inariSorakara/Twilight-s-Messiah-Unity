...
[CreateAssetMenu(fileName = "New Copper Event", menuName = "Twilight's Messiah/Events/Copper Event")]
public class CopperEventSO : EventTypeSO
{
    ...
    {
        Debug.Log($"Copper Event triggered by {player.name}!");
        ...
        if (selectedEnemy == null)
        {
            ...
            if (selectedEnemy == null)
            {
                Debug.Log("No enemies left in this Copper Event's pool, not even challengers!");
                return;
            }
    ...
    private void HandleBattleComplete(GameObject player, GameObject enemy, bool playerWon)
    {
        ...
        if (playerWon)
        {
            Debug.Log("Player won the Copper Event battle!");
        ...
        {
            Debug.Log("Player lost the Copper Event battle!");
